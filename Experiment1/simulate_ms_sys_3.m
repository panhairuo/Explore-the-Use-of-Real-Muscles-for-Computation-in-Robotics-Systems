...
%
%  [1] Hauser, H.; Ijspeert, A.; F點hslin, R.; Pfeifer, R. & Maass, W.
...
% 
%  [2] Hauser, H.; Ijspeert, A.; F點hslin, R.; Pfeifer, R. & Maass, W.
...
%  	   Fd=0; %no damping
	...
	% force amplitudes (NONlinear)
	    % 计算相对速度
        delta_d = (d - W.dist_old(c,1));       % 位移变化
        delta_d_dt = delta_d / time_step;      % 速度
        ...
        % 计算迟滞变量的变化率
        dh_dt = A*delta_d_dt - beta*abs(delta_d_dt)*abs(h)^(n-1)*h - gamma*delta_d_dt*abs(h)^n;
        
        % 更新迟滞变量 (欧拉积分)
        W.h(c,1) = h + dh_dt * time_step;
        
        % 计算包含迟滞的弹簧力
        alpha = W.bw_alpha(c,1);
        Fk = +( W.k3(c,1)*max(0,(d-W.l0(c,1)))^3 + W.k1(c,1)*max(0,(d-W.l0(c,1))) + alpha * W.h(c,1));
